...
class Nacos:
    ...
    def restartProcess(self, name):
        print('restartProcess name:{0}'.format(name))
        try:
            ...
            while True:
                info = rpcinterface.supervisor.getProcessInfo(name)
            ...
            while True:
                info = rpcinterface.supervisor.getProcessInfo(name)
                if info['state'] != ProcessStates.STARTING and info['state'] != ProcessStates.RUNNING:
