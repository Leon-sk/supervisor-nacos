...
class Options:
    ...
    def add(self,
        ...
        if flag is not None:
            ...
            def handler(arg, flag=flag): return flag
    ...
    def realize(self, args=None, doc=None, progname=None):
        ...
        if self.args and not self.positional_args_allowed:
            self.usage("positional arguments are not supported: %s" % 
                       (str(self.args)))
...
class ServerOptions(Options):
    ...
    def read_config(self, fp):
        ...
        self.nacos_config = self.nacos_config_from_parser(parser)
    ...
    def _processes_from_section(self, parser, section, group_name,
        ...
        stdout_cmaxbytes = byte_size(
            get(section, 'stdout_capture_maxbytes', '0'))
        stdout_events = boolean(get(section, 'stdout_events_enabled', 'false'))
        stderr_cmaxbytes = byte_size(
            get(section, 'stderr_capture_maxbytes', '0'))
        stderr_events = boolean(get(section, 'stderr_events_enabled', 'false'))
        serverurl = get(section, 'serverurl', None)
        if serverurl and serverurl.strip().upper() == 'AUTO':
            serverurl = None
        if  '127.0.0.1' in serverurl or 'localhost' in serverurl:
            ip, port = serverurl.strip('http://').split(':')
            ip = self.get_host_ip()
            serverurl = 'http://{0}:{1}'.format(ip, port)
        nacosconfig = get(section, 'nacosconfig', None)
        nacosgroup = get(section, 'nacosgroup', None)
        if not nacosgroup and "nacosgroup" in os.environ:
            nacosgroup = os.environ["nacosgroup"]
        ...
        for process_num in range(numprocs_start, numprocs + numprocs_start):
            ...
            expansions.update(
                {'process_num': process_num, 'numprocs': numprocs})
            expansions.update(self.environ_expansions)
            ...
            pconfig = klass(
                ...
                serverurl=serverurl, nacosconfig=nacosconfig, nacosgroup=nacosgroup)
    ...
    def server_configs_from_parser(self, parser):
        ...
        if inet_serverdefs:
            self.nacos_config['metadata'] = metadata
    ...
    def nacos_config_from_parser(self, parser):
        ...
        for name, section in nacos:
            ...
            if not server_addresses and "server_addresses" in os.environ:
    ...
    def opennacos(self, supervisord):
        try:
            self.nacos = nacosd.Nacos(supervisord)
            self.nacos.start()
...
class ClientOptions(Options):
    ...
    def __init__(self):
        ...
        self.add("history", "supervisorctl.history_file",
                 "r:", "history_file=")
...
class ProcessConfig(Config):
    req_param_names = [
        ...
        'exitcodes', 'redirect_stderr', 'nacosgroup', 'nacosconfig']
